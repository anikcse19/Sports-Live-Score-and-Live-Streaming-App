...
const FootballMatches = () => {
  ...
  const [filteredInPlayFootballMtaches, setFilteredInPlayFootballMatches] =
    useState([]);
  ...
  const fetchFootballMatches = async () => {
    try {
      axios
        .get(`${baseUrl}/api/event?sport=SOCCER&site=${supportSite}`, {
          headers: {
        ...
        .then((res) => {
          if (res?.data?.message === "success") {
            setFootballMatches(res?.data?.data?.competitionEvents);
          }
        });
    } catch (error) {
  ...
  useEffect(() => {
    ...
    setFilteredInPlayFootballMatches(filteredMatches);
  }, [footballMatches]);
  ...
  return (
    <div className="flex flex-col gap-4 px-4 pt-3 py-3">
      ...
      <div className="">
          ...
          <div
            onClick={() => {
              if (openAllMatchDetails) {
                ...
                setOpenMatchDetails(
                  filteredInPlayFootballMtaches.map(
        ...
        {filteredInPlayFootballMtaches.length === 0 && (
        ...
        <div className="flex flex-col gap-y-4">
          {filteredInPlayFootballMtaches?.map((match, i) => (
            <div
              key={i}
              className={`${
                ...
                <div key={i}>
                  ...
                  {event?.matchOdds !== null && (
                    <div className="p-3 flex flex-col gap-y-0.5">
                      <div className="bg-yellow-400 py-1 px-3 rounded-tl-xl rounded-tr-xl flex items-center justify-between gap-0.5">
                        <div className="flex items-center gap-1 md:gap-x-3 w-[70%]  border-r-2 border-black">
                          <p className="text-sm md:text-base">Match Odds</p>
                          <p className="text-xs">
                            Mathced {event.matchOdds?.totalMatched}
                          </p>
                        </div>
                        <div className="w-[15%] text-center text-sm md:text-base">
                          Back
                        </div>
                        <div className="w-[15%] text-center text-sm md:text-base">
                          Lay
